...
import type { Form, FormSubmission, FormField } from "@shared/schema";
...
export default function ReportsPage() {
  ...
  const { data: allSubmissions, isLoading: subsLoading } = useQuery<FormSubmission[]>({ queryKey: ["/api/submissions"] });
  const [selectedFormId, setSelectedFormId] = useState<string>("");

  const isLoading = formsLoading || subsLoading;

  const selectedForm = forms?.find((f) => f.id === selectedFormId);
  const submissions = allSubmissions?.filter((s) => s.formId === selectedFormId) || [];
  ...
  const getFieldSummary = (field: FormField) => {
    ...
    if (field.type === "checkbox") {
      ...
      return [
        ...
        { name: "No", value: values.length - trueCount },
      ];
    }
    return null;
  };
  ...
  const exportCSV = () => {
    ...
    const rows = submissions.map((s) =>
      formFields.map((f) => {
        const val = (s.data as Record<string, any>)?.[f.id];
    ...
    a.href = url;
    a.download = `${selectedForm?.name || "report"}.csv`;
    a.click();
  ...
  const submissionsByForm = forms?.map((f) => ({
    ...
    submissions: allSubmissions?.filter((s) => s.formId === f.id).length || 0,
  })) || [];

  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="space-y-1">
        <h1 className="text-2xl font-serif font-bold" data-testid="text-reports-title">Reports</h1>
      ...
      <Card className="p-6">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <Select value={selectedFormId} onValueChange={setSelectedFormId}>
              <SelectTrigger className="w-64" data-testid="select-report-form">
        ...
        {!selectedFormId ? (
          ...
          <div className="space-y-8">
            {formFields.some((f) => f.type === "dropdown" || f.type === "checkbox") && (
              <div className="grid sm:grid-cols-2 gap-6">
                {formFields
                  .filter((f) => f.type === "dropdown" || f.type === "checkbox")
                  .map((field) => {
                  ...
                  <TableBody>
                    {submissions.map((sub, index) => (
                      <TableRow key={sub.id}>
                        ...
                        {formFields.map((f) => (
                          <TableCell key={f.id}>
                            {String((sub.data as Record<string, any>)?.[f.id] ?? "-")}
                          </TableCell>
                        ))}
