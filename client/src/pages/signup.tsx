      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className={`h-1.5 flex-1 rounded-full transition-colors ${
            i <= passed
              ? passed <= 2 ? "bg-destructive" : passed <= 3 ? "bg-chart-4" : "bg-chart-2"
              : "bg-muted"
          }`} />
        ))}
      </div>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-md text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Already confirmed? <a href="/login" className="text-foreground font-medium hover:underline">Log in</a></p>
        </div>
      </div>
    );
  }
        ...
        <Card className="p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input id="confirm-password" type={showConfirmPassword ? "text" : "password"} placeholder="Confirm your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
