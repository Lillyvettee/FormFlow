...
export default function SubmitFormPage() {
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      return apiRequest("POST", "/api/submissions", { formId, data: formData });
    },
    onSuccess: () => {
      setSubmitted(true);
    ...
    onError: (error: Error) => {
      if (isUnauthorizedError(error)) {
        toast({ title: "Unauthorized", description: "Please log in to submit this form.", variant: "destructive" });
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <Card className="w-full max-w-md p-8 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">This form may have been removed or you don't have access.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="space-y-1">
        <h1 className="text-2xl font-serif font-bold" data-testid="text-submit-form-title">{form.name}</h1>
            ...
            <div key={field.id} className="space-y-2">
              ...
              {field.type === "text" && (
                <Input
                  ...
                  data-testid={`input-submit-${field.id}`}
                />
              )}
              ...
              {field.type === "number" && (
                <Input
                  ...
                  placeholder="0"
                  value={formData[field.id] ?? ""}
                  onChange={(e) => updateField(field.id, e.target.value ? parseFloat(e.target.value) : "")}
                  data-testid={`input-submit-${field.id}`}
                />
              )}
              ...
              {field.type === "checkbox" && (
                <div className="flex items-center gap-2">
                  <Checkbox
                    ...
                    data-testid={`checkbox-submit-${field.id}`}
                  />
              ...
              {field.type === "dropdown" && (
                ...
                >
                  <SelectTrigger data-testid={`select-submit-${field.id}`}>
