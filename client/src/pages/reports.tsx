...
const COLORS = [
  ...
  "hsl(340, 70%, 45%)", "hsl(40, 85%, 40%)", "hsl(270, 60%, 40%)"
];
...
const clearFilters = () => { setSearchQuery(""); setDateFrom(""); setDateTo(""); };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      const [formsRes, donationsRes, volunteerRes, inventoryRes] = await Promise.all([
        supabase.from("forms").select("*").eq("user_id", user.id).eq("is_archived", false),
        supabase.from("donations").select("*").eq("user_id", user.id).order("date", { ascending: true }),
        supabase.from("volunteer_hours").select("*").eq("user_id", user.id).order("date", { ascending: true }),
  ...
  const donationsByMonth = donations.reduce((acc, d) => {
    const key = new Date(d.date + "T00:00:00").toLocaleDateString("en-US", { month: "short", year: "2-digit" });
    acc[key] = (acc[key] ?? 0) + Number(d.amount);
    return acc;
  }, {} as Record<string, number>);
  const donationsByMonthData = Object.entries(donationsByMonth).map(([month, total]) => ({ month, total }));

  const donationsByCampaign = donations.reduce((acc, d) => {
    const key = d.campaign ?? "General";
    acc[key] = (acc[key] ?? 0) + Number(d.amount);
    return acc;
  }, {} as Record<string, number>);
  const campaignData = Object.entries(donationsByCampaign).map(([name, value]) => ({ name, value }));
  ...
  const methodData = Object.entries(donationsByMethod).map(([name, value]) => ({ name, value }));

  // ── Volunteer helpers ──
  const totalHours = volunteerHours.reduce((s, v) => s + Number(v.hours), 0);
  const hoursByVolunteer = volunteerHours.reduce((acc, v) => {
    acc[v.volunteer_name] = (acc[v.volunteer_name] ?? 0) + Number(v.hours);
    return acc;
  }, {} as Record<string, number>);
  const topVolunteers = Object.entries(hoursByVolunteer)
    .map(([name, hours]) => ({ name: name.length > 14 ? name.slice(0, 14) + "…" : name, hours }))
    .sort((a, b) => b.hours - a.hours).slice(0, 10);

  const hoursByActivity = volunteerHours.reduce((acc, v) => {
    acc[v.activity] = (acc[v.activity] ?? 0) + Number(v.hours);
    return acc;
  }, {} as Record<string, number>);
  const activityData = Object.entries(hoursByActivity).map(([name, value]) => ({ name, value }));

  const hoursByMonth = volunteerHours.reduce((acc, v) => {
    const key = new Date(v.date + "T00:00:00").toLocaleDateString("en-US", { month: "short", year: "2-digit" });
    acc[key] = (acc[key] ?? 0) + Number(v.hours);
    return acc;
  }, {} as Record<string, number>);
  const hoursByMonthData = Object.entries(hoursByMonth).map(([month, hours]) => ({ month, hours }));
  ...
  const getFieldSummary = (field: FormField) => {
    ...
    if (field.type === "number") {
      ...
      return { type: "number", avg: (nums.reduce((a, b) => a + b, 0) / nums.length).toFixed(1), min: Math.min(...nums), max: Math.max(...nums) };
    }
  ...
  const exportCSV = () => {
    ...
    if (reportType === "donations") {
      csv = ["Donor,Amount,Date,Campaign,Method,Notes",
        ...donations.map(d => `"${d.donor_name}","${d.amount}","${d.date}","${d.campaign ?? ""}","${d.payment_method ?? ""}","${d.notes ?? ""}"`)
      ].join("\n");
    } else if (reportType === "volunteers") {
      csv = ["Volunteer,Activity,Hours,Date,Notes",
        ...volunteerHours.map(v => `"${v.volunteer_name}","${v.activity}","${v.hours}","${v.date}","${v.notes ?? ""}"`)
      ].join("\n");
    } else if (reportType === "inventory") {
      csv = ["Name,Category,Quantity,Unit,Condition,Location",
        ...inventory.map(i => `"${i.name}","${i.category ?? ""}","${i.quantity}","${i.unit ?? ""}","${i.condition ?? ""}","${i.location ?? ""}"`)
      ].join("\n");
    } else if (reportType === "forms" && submissions.length && formFields.length) {
      csv = ["Submitted At", ...formFields.map(f => f.label)].join(",") + "\n" +
        submissions.map(s => [new Date(s.submitted_at).toLocaleString(), ...formFields.map(f => `"${(s.data as any)?.[f.id] ?? ""}"`)].join(",")).join("\n");
    }
    if (!csv) return;
  ...
  const exportPDF = () => {
    ...
    if (!w) { setIsExporting(false); return; }
    ...
    w.document.close();
    setTimeout(() => { w.print(); setIsExporting(false); }, 600);
  };
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Visual reports for your board, donors, and grant applications</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={exportPDF} disabled={isExporting}>
            <FileDown className="mr-1 h-3.5 w-3.5" /> {isExporting ? "Preparing…" : "Export PDF"}
          </Button>
        </div>
      </div>
      ...
      {reportType === "impact" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Total Raised", value: `$${totalDonations.toLocaleString()}`, icon: DollarSign, color: "text-chart-2", bg: "bg-chart-2/10" },
              { label: "Volunteer Hours", value: totalHours.toLocaleString(), icon: Clock, color: "text-chart-3", bg: "bg-chart-3/10" },
              { label: "Form Responses", value: allSubmissions.length, icon: FileText, color: "text-primary", bg: "bg-primary/10" },
              { label: "Inventory Items", value: inventory.length, icon: Package, color: "text-chart-4", bg: "bg-chart-4/10" },
            ].map(stat => (
                ...
                <div className="h-52">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={donationsByMonthData}>
                      ...
                      <Tooltip contentStyle={tooltipStyle} formatter={(v) => [`$${Number(v).toLocaleString()}`, "Donations"]} />
                      <Area type="monotone" dataKey="total" stroke={COLORS[0]} fill="url(#donGrad)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              ) : <p className="text-sm text-muted-foreground text-center py-12">No donation data yet</p>}
            </Card>
                ...
                <div className="h-52">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={hoursByMonthData}>
                      ...
                      <Bar dataKey="hours" fill={COLORS[2]} radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ) : <p className="text-sm text-muted-foreground text-center py-12">No volunteer data yet</p>}
            </Card>
          </div>
          ...
          {lowStockItems.length > 0 && (
            <Card className="p-5 border-destructive/40 bg-destructive/5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-destructive">Low Stock Items</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {lowStockItems.map(i => (
                  <Badge key={i.id} variant="destructive">{i.name}: {i.quantity} left</Badge>
                ))}
              </div>
            </Card>
          )}
      ...
      {reportType === "donations" && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { label: "Total Raised", value: `$${totalDonations.toLocaleString("en-US", { minimumFractionDigits: 2 })}` },
              { label: "Donations", value: donations.length },
              { label: "Unique Donors", value: new Set(donations.map(d => d.donor_name)).size },
              { label: "Avg Donation", value: donations.length ? `$${(totalDonations / donations.length).toFixed(2)}` : "$0" },
            ].map(s => (
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={donationsByMonthData}>
                      ...
                      <Area type="monotone" dataKey="total" stroke={COLORS[0]} fill="url(#donGrad2)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              ...
              <Card className="p-6">
                ...
                {campaignData.length > 0 ? (
                  <div className="h-36">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={campaignData} cx="50%" cy="50%" innerRadius={28} outerRadius={52} dataKey="value" paddingAngle={2}>
                ...
                <div className="space-y-1 mt-2">
                  {campaignData.slice(0, 4).map((c, i) => (
                    <div key={c.name} className="flex items-center justify-between text-xs gap-2">
                      <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ backgroundColor: COLORS[i % COLORS.length] }} /><span className="truncate">{c.name}</span></div>
                ...
                <div className="space-y-2">
                  {methodData.map((m, i) => (
                    <div key={m.name} className="flex items-center justify-between text-xs gap-2">
                      <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: COLORS[i % COLORS.length] }} /><span>{m.name}</span></div>
                      <span className="font-medium">{m.value}</span>
                    </div>
                  ))}
                  {!methodData.length && <p className="text-xs text-muted-foreground text-center py-2">No data</p>}
                </div>
              </Card>
            </div>
          </div>
                ...
                <TableBody>
                  {donations.slice().reverse().map(d => (
                    <TableRow key={d.id}>
                      ...
                      <TableCell className="text-right text-primary font-semibold">${Number(d.amount).toLocaleString("en-US", { minimumFractionDigits: 2 })}</TableCell>
          ...
          {!donations.length && <div className="text-center py-16 text-sm text-muted-foreground">No donations recorded yet</div>}
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: "Volunteers", value: new Set(volunteerHours.map(v => v.volunteer_name)).size },
              { label: "Sessions", value: volunteerHours.length },
              { label: "Avg Session", value: volunteerHours.length ? `${(totalHours / volunteerHours.length).toFixed(1)} hrs` : "—" },
            ].map(s => (
              <Card key={s.label} className="p-5 text-center">
                <p className="text-2xl font-bold text-primary">{s.value}</p>
                <p className="text-xs text-muted-foreground mt-1">{s.label}</p>
              ...
              {activityData.length > 0 ? (
                <>
                  <div className="h-40">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={activityData} cx="50%" cy="50%" innerRadius={32} outerRadius={60} dataKey="value" paddingAngle={2}>
                  ...
                  <div className="space-y-1 mt-2">
                    {activityData.slice(0, 5).map((a, i) => (
                      <div key={a.name} className="flex items-center justify-between text-xs gap-2">
                        <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: COLORS[i % COLORS.length] }} /><span className="truncate">{a.name}</span></div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: "Out of Stock", value: inventory.filter(i => i.quantity === 0).length },
              { label: "Categories", value: Object.keys(byCategory).length },
            ].map(s => (
              <Card key={s.label} className={`p-5 text-center ${s.label === "Low Stock" && s.value > 0 ? "border-destructive/40" : ""}`}>
                <p className={`text-2xl font-bold ${s.label === "Low Stock" && s.value > 0 ? "text-destructive" : "text-primary"}`}>{s.value}</p>
                <p className="text-xs text-muted-foreground mt-1">{s.label}</p>
              </Card>
            ))}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <Card className="p-6">
              ...
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={inventory.map(i => ({ name: i.name.length > 12 ? i.name.slice(0, 12) + "…" : i.name, quantity: i.quantity, low: i.quantity <= (i.low_stock_threshold ?? 5) }))}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                    <XAxis dataKey="name" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} angle={-30} textAnchor="end" interval={0} />
                    <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} />
                    <Tooltip contentStyle={tooltipStyle} />
                    <Bar dataKey="quantity" radius={[4, 4, 0, 0]}>
                      {inventory.map((item, i) => (
                        <Cell key={i} fill={item.quantity <= (item.low_stock_threshold ?? 5) ? "hsl(var(--destructive))" : COLORS[0]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              {categoryData.length > 0 ? (
                <>
                  <div className="h-40">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={categoryData} cx="50%" cy="50%" innerRadius={32} outerRadius={60} dataKey="value" paddingAngle={2}>
                  ...
                  <div className="space-y-1 mt-2">
                    {categoryData.map((c, i) => (
                      <div key={c.name} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1.5"><div className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: COLORS[i % COLORS.length] }} /><span>{c.name}</span></div>
          ...
          {lowStockItems.length > 0 && (
            <Card className="p-5 border-destructive/40 bg-destructive/5">
              <div className="flex items-center gap-2 mb-3">
                <AlertTriangle className="h-4 w-4 text-destructive" />
                <h3 className="font-semibold text-destructive">Needs Restocking</h3>
              </div>
              <div className="overflow-auto">
                <Table>
                  <TableHeader><TableRow><TableHead>Item</TableHead><TableHead>Qty</TableHead><TableHead>Threshold</TableHead><TableHead>Category</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {lowStockItems.map(i => (
                      <TableRow key={i.id}>
                        <TableCell className="font-medium">{i.name}</TableCell>
                        <TableCell><Badge variant="destructive">{i.quantity}</Badge></TableCell>
                        <TableCell className="text-muted-foreground">{i.low_stock_threshold ?? 5}</TableCell>
        ...
        <div className="space-y-1.5">
          ...
          <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="h-9 text-sm" />
                    ...
                    return (
                      <Card key={field.id} className="p-6">
                        ...
                        <div className="h-44">
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie data={d} cx="50%" cy="50%" innerRadius={35} outerRadius={62} paddingAngle={2} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
              ...
              {formFields.filter(f => f.type === "number").map(field => {
                ...
                return (
                  <Card key={field.id} className="p-6">
                    <h4 className="font-semibold mb-3">{field.label} — Statistics</h4>
                    <div className="grid grid-cols-3 gap-3">
                      {[["Average", summary.avg], ["Min", summary.min], ["Max", summary.max]].map(([l, v]) => (
                        <div key={l as string} className="text-center p-4 bg-muted/40 rounded-md">
